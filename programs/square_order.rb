class SquareOrder
  ...
  def self.order(list)
    ...
    # use bubble sort in result array
    0.upto(result.length - 1) do |t|
      0.upto(result.length - 2 - t) do |i|
...
test_lists.each do |list|
  ...
  #   if execution_time > 2
  #     puts "Test case #{index+1} failed: used #{execution_time} seconds, exceeding time limit."
  #   else
